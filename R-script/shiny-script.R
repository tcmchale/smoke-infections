...
# Join covid_data_monthly and cal_pop
covid_data_monthly <- covid_data_monthly %>%
  left_join(cal_pop, by = "NAME") %>%
  rename(population=estimate) %>%
  mutate(cases_per_10000 = cases/population*10000,
         deaths_per_10000 = deaths/population*10000) %>%
  select(-c(variable, moe))
st_write(covid_data_monthly, "covid_data_monthly.shp", driver = "ESRI Shapefile")
  ...
  observe({
    ...
    leafletProxy("smoke_map", data = filtered_data) %>%
      ...
      addPolygons(
        ...
        label = ~paste0("<strong>County: </strong>", filtered_data$NAME, "<br>",
                        "<strong>Mean Smoke Exposure (PM2.5): </strong>", format(filtered_data$mean_smoke_exposure, big.mark = ",")),
        labelOptions = labelOptions(direction = "auto")
      ) %>%
      addLegend(
        pal = pal,
